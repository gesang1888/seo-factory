#!/usr/bin/env python3
"""Build Lovegobuy cluster — 8 domains."""
...
from scripts.fetch_lovegobuy_activities import refresh_activities  # noqa: E402
from scripts.pages_lovegobuy import get_page, page_allowed  # noqa: E402
from scripts.renderer_lovegobuy import (  # noqa: E402
    copy_assets,
    render_page,
    write_404_page,
    write_redirect_page,
)
from scripts.trust_pages_lovegobuy import TRUST_SLUGS, get_trust_page  # noqa: E402
...
def build_slug_index() -> dict[str, set[str]]:
    ...
    for domain, meta in LOVEGOBUY_DOMAINS.items():
        region = meta["region"]
        role = meta.get("role", "spreadsheet")
        slugs = set()
        for slug in domain_slugs(region, role):
            page = get_page(slug)
...
def write_plural_redirects() -> None:
...
def main() -> None:
    if not CONFIG_PATH.is_file():
        print("Missing site.lovegobuy.config.json", file=sys.stderr)
        sys.exit(1)

    print("Syncing Lovegobuy.com homepage activities...")
    refresh_activities()
    print("Fetching live W2CLinks products...")
    ...
    write_plural_redirects()

    manifest = {
        "built": TODAY,
        "cluster": "lovegobuy",
        "main_site": "w2clinks.com",
        "invite": "https://www.lovegobuy.com/?invite_code=W5RJX3",
        "domains": list(LOVEGOBUY_DOMAINS.keys()),
        "pages_per_domain": domain_stats,
        "total_pages": total_pages,
        "live_products": len(live_cache.get("products", [])),
    }
    (DIST / "build-lovegobuy-manifest.json").write_text(
        json.dumps(manifest, indent=2), encoding="utf-8"
    )

    print(f"\nBuilt {total_pages} pages across {len(LOVEGOBUY_DOMAINS)} Lovegobuy domains")
    print(f"Output: {DIST}")
