#!/usr/bin/env python3
"""Build Kakobuy cluster: kakospreadsheet.{es,fr,nl,ca} + kakobuy.fi"""
...
from scripts.kakobuy_ca_local import build_kakobuy_ca  # noqa: E402
...
from scripts.trust_pages_kakobuy import TRUST_SLUGS, get_trust_page  # noqa: E402

DIST = ROOT / "dist"
TEMPLATE_ROOT = ROOT / "templates"
KAKOBUY_CONFIG = ROOT / "site.kakobuy.config.json"
TODAY = date.today().isoformat()


def load_kakobuy_config() -> dict:
    return json.loads(KAKOBUY_CONFIG.read_text(encoding="utf-8"))


def build_slug_index() -> dict[str, set[str]]:
    index: dict[str, set[str]] = {}
    for domain, meta in KAKOBUY_DOMAINS.items():
        region = meta["region"]
        slugs = set()
...
def build_domain(
    ...
    if domain == "kakospreadsheet.ca":
        return build_kakobuy_ca(DIST / domain)
    if domain == "kakospreadsheet.es":
...
def main() -> None:
    if not KAKOBUY_CONFIG.is_file():
        print("Missing site.kakobuy.config.json", file=sys.stderr)
        sys.exit(1)

    print("Fetching live W2CLinks products + Kakobuy.com activities...")
    live_cache = refresh_cache()
    refresh_activities()
    site_config = load_kakobuy_config()
    ...
    manifest = {
        ...
        "domains": list(KAKOBUY_DOMAINS.keys()),
        "pages_per_domain": domain_stats,
        "total_pages": total_pages,
        "live_products": len(live_cache.get("products", [])),
    }
    (DIST / "build-kakobuy-manifest.json").write_text(
        json.dumps(manifest, indent=2), encoding="utf-8"
    )

    print(f"\nBuilt {total_pages} pages across {len(KAKOBUY_DOMAINS)} Kakobuy domains")
    print(f"Output: {DIST}")
