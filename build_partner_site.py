#!/usr/bin/env python3
"""Build OrientDig partner cluster: orientdig.us / .es / .fr / .at"""
...
from scripts.domains_partner import PARTNER_DOMAINS, domain_slugs, slug_to_path  # noqa: E402
...
PARTNER_CONFIG = ROOT / "site.partner.config.json"
TODAY = date.today().isoformat()


def load_partner_config() -> dict:
    return json.loads(PARTNER_CONFIG.read_text(encoding="utf-8"))


def build_slug_index() -> dict[str, set[str]]:
    index: dict[str, set[str]] = {}
    for domain, meta in PARTNER_DOMAINS.items():
        region = meta["region"]
        slugs = set()
...
def main() -> None:
    if not PARTNER_CONFIG.is_file():
        print("Missing site.partner.config.json", file=sys.stderr)
        sys.exit(1)

    print("Fetching live W2CLinks products + OrientDig help data...")
    live_cache = refresh_cache()
    site_config = load_partner_config()
    ...
    manifest = {
        "built": TODAY,
        "cluster": "orientdig-partner",
        "domains": list(PARTNER_DOMAINS.keys()),
        "pages_per_domain": domain_stats,
        "total_pages": total_pages,
        "live_products": len(live_cache.get("products", [])),
    }
    (DIST / "build-partner-manifest.json").write_text(
        json.dumps(manifest, indent=2), encoding="utf-8"
    )

    print(f"\nBuilt {total_pages} pages across {len(PARTNER_DOMAINS)} partner domains")
    print(f"Output: {DIST}")
