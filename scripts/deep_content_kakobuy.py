...
def _base_spreadsheet(region: str, lang: str) -> list[tuple[str, str]]:
    en = [
        (
            "What kakobuy spreadsheet searchers actually want",
            "Most people typing kakobuy spreadsheet are not looking for a downloadable Excel file on a random blog. "
        ...
        (
            ...
            "When a find looks promising, open the listing, copy the source link, and paste it into Kakobuy. "
        ...
        (
            ...
            "size charts, or Kakobuy warehouse inspection. Treat finds as a starting point, not a guarantee.",
        ),
        (
        ...
        (
            ...
            f"After you pick items on W2CLinks, verify Kakobuy promotions in your account, read QC photos carefully, "
    ...
    if lang == "es":
        return [
            (
                "Qué buscan realmente con kakobuy spreadsheet",
            ...
            (
                ...
                "y pégalo en Kakobuy. Paga, espera inbound, revisa QC y solo entonces crea el paquete internacional.",
            ),
            (
                "Errores comunes",
    ...
    if lang == "fr":
        return [
            (
                "Ce que les recherches kakobuy spreadsheet signifient",
            ...
            (
                "Workflow en étapes",
                "Ouvrez le tableur, triez par newest, filtrez catégorie/marque, copiez le lien source dans Kakobuy, "
    ...
    if lang == "de":
        return [
            (
                "Was Nutzer mit kakobuy spreadsheet meinen",
            ...
            (
                "Schritt-für-Schritt",
                "Spreadsheet öffnen, nach newest sortieren, Kategorie/Marke filtern, Link in Kakobuy einfügen, "
    ...
    if lang == "nl":
        return [
            (
                "Wat zoekers met kakobuy spreadsheet bedoelen",
            ...
            (
                "Stap-voor-stap",
                "Open het spreadsheet, sorteer op newest, filter categorie of merk, kopieer de bronlink naar Kakobuy, "
            ...
            (
                ...
                "promoties op Kakobuy.com (o.a. 3000 CNY bundel) wijzigen; controleer altijd in je dashboard.",
            ),
        ] + en[2:]
    if lang == "fi":
        return [
            (
                "Mitä kakobuy spreadsheet -hakijat oikeasti haluavat",
            ...
            (
                ...
                "Avaa spreadsheet, järjestä newest, suodata kategoria tai brändi, kopioi linkki Kakobuyhin, "
            ...
            (
                ...
                "Kakobuy.comin kampanjat (esim. 3000 CNY) muuttuvat; vahvista aina tililläsi.",
            ),
        ] + en[2:]
    return en
...
def _shipping_deep(region: str, lang: str) -> list[tuple[str, str]]:
    en = [
        (
            ...
            f"International shipping does not start when you pay for a Taobao/1688 item. Items land at the Kakobuy "
        ...
        (
            ...
            "Customs delays are often unrelated to Kakobuy itself — they happen at import screening. "
...
def _trust_deep(lang: str) -> list[tuple[str, str]]:
    return [
        (
            ...
            f"Kakobuy ({PLATFORM}) purchases from Chinese marketplaces on your behalf. "
        ...
        (
            ...
            "and open live finds — then complete orders on Kakobuy.",
        ),
    ]
...
def _coupons_deep(lang: str) -> list[tuple[str, str]]:
    return [
        (
            "How Kakobuy coupons actually work",
            ...
            "Always verify eligibility inside your Kakobuy account before paying.",
        ),
        (
        ...
        (
            "¥3000 new-user coupon (synced with Kakobuy.com)",
            "Kakobuy.com currently promotes a ¥3000 coupon bundle for new registrations — amounts and eligibility "
            "change with campaigns. Register via our invitation link "
            f'<a href="{REGISTER}" target="_blank" rel="sponsored noopener">{REGISTER}</a>, '
            "then confirm the live bonus inside your Kakobuy wallet before paying. "
            "Older Reddit threads may still mention 1300CNY; treat those as outdated unless the help center confirms them.",
        ),
        (
            ...
            "They do not replace Kakobuy's own terms — shipping fees, QC, and line rules still apply. "
...
def _howto_deep(lang: str) -> list[tuple[str, str]]:
    return [
        (
            ...
            f"1) Discover on W2CLinks spreadsheet. 2) Copy product URL. 3) Paste into Kakobuy buy form. "
...
SLUG_HANDLERS = {
    ...
    "kakobuy-spreadsheet": lambda r, l: _base_spreadsheet(r, l),
    "kakobuy-spreadsheets": lambda r, l: _base_spreadsheet(r, l),
    "kakobuy-finds": lambda r, l: _base_spreadsheet(r, l) + [
        (
    ...
    "kakobuy-shipping": lambda r, l: _shipping_deep(r, l),
    "kakobuy-shipping-calculator": lambda r, l: _shipping_deep(r, l),
    "kakobuy-shipping-coupons": lambda r, l: _shipping_deep(r, l) + _coupons_deep(l),
    "livraison-kakobuy": lambda r, l: _shipping_deep(r, l),
    "envio-kakobuy-espana": lambda r, l: _shipping_deep(r, l),
    "kakobuy-toimitus": lambda r, l: _shipping_deep(r, l),
    "kakobuy-shipping-to-canada": lambda r, l: _shipping_deep(r, l),
    "kakobuy-coupons": lambda r, l: _coupons_deep(l),
    "kakobuy-coupon": lambda r, l: _coupons_deep(l),
    "kakobuy-codes": lambda r, l: _coupons_deep(l),
    "kakobuy-qc": lambda r, l: _qc_deep(l),
    "qc-kakobuy": lambda r, l: _qc_deep(l),
    "is-kakobuy-legit": lambda r, l: _trust_deep(l),
    "is-kakobuy-safe": lambda r, l: _trust_deep(l),
    "how-to-use-kakobuy": lambda r, l: _howto_deep(l),
    "como-comprar-en-kakobuy": lambda r, l: _howto_deep(l),
    "kakobuy-review": lambda r, l: _trust_deep(l),
    "avis-kakobuy": lambda r, l: _trust_deep(l),
    "best-kakobuy-spreadsheet": lambda r, l: _base_spreadsheet(r, l),
}
