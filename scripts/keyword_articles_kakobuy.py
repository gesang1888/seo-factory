...
from scripts.country_seo_kakobuy import top_keywords
from scripts.hub_icons import flag_emoji, region_label
from scripts.link_helpers_kakobuy import main_spreadsheet_url, product_search_url

SPREADSHEET = main_spreadsheet_url()

# Internal page hints per keyword theme
PAGE_LINKS = {
    "spreadsheet": "kakobuy-spreadsheet/",
    "finds": "kakobuy-finds/",
    "shipping": "kakobuy-shipping/",
    "coupons": "kakobuy-coupons/",
    "legit": "is-kakobuy-legit/",
    "safe": "is-kakobuy-safe/",
    "howto": "how-to-use-kakobuy/",
    "discord": "kakobuy-discord/",
    "coupon_fr": "kakobuy-coupon/",
    "spreadsheet_fr": "spreadsheet-kakobuy/",
    "avis": "avis-kakobuy/",
    "como_comprar": "como-comprar-en-kakobuy/",
    "shipping_coupons_es": "kakobuy-shipping-coupons/",
    "best_spreadsheet": "best-kakobuy-spreadsheet/",
    "calculator": "kakobuy-shipping-calculator/",
    "payment": "kakobuy-payment-methods/",
    "erfahrungen": "kakobuy-erfahrungen/",
    "trustpilot": "kakobuy-trustpilot/",
    "codes": "kakobuy-codes/",
    "qc": "kakobuy-qc/",
}
...
def article_us(home_prefix: str) -> str:
    return (
        _section(
            "What US shoppers mean when they search Kakobuy",
            [
                "The brand query kakobuy usually signals intent to understand the agent itself — registration, "
                "balance top-ups, warehouse flow and whether the platform fits a first purchase. "
                "It is different from kakobuy spreadsheet, which almost always means a browsable finds list.",
                "US search volume also clusters around trust queries such as is kakobuy legit and is kakobuy safe. "
        ...
        + _section(
            ...
            [
                f"kakobuy spreadsheet and best kakobuy spreadsheet map to the same practical action: open the live "
        ...
        + _section(
            ...
            [
                "kakobuy shipping calculator searches reflect parcel planning anxiety: weight, line choice, and whether "
                f"delivery fits a deadline. Read {_link(home_prefix, PAGE_LINKS['shipping'], 'US shipping notes')} for "
                f"estimated line ranges, then confirm live quotes inside Kakobuy before paying.",
                "Coupon-related queries such as kakobuy coupon and kakobuy coupon codes change frequently. "
        ...
        + _section(
            ...
            [
                "1) Shortlist on W2CLinks. 2) Paste links into Kakobuy. 3) Review QC photos. "
                "4) Compare shipping lines with realistic customs buffer. 5) Track import milestones. "
                f"Full steps live in {_link(home_prefix, PAGE_LINKS['howto'], 'how to use Kakobuy')}.",
            ],
        )
    )
...
def article_fr(home_prefix: str) -> str:
    return (
        _section(
            "Ce que les acheteurs français cherchent avec « kakobuy »",
            [
                "En France, la requête kakobuy mélange curiosité sur la plateforme agent et envie de trouver des finds "
                "rapidement. Ce n'est pas la peine de confondre la marque avec un tableur local : le browse structuré "
                f"passe par W2CLinks, tandis que la commande se finalise sur kakobuy.com.",
                "Les recherches kakobuy spreadsheet et spreadsheet kakobuy (ordre inversé) pointent vers le même "
                f"besoin : filtrer des listes communautaires. Utilisez {_link(home_prefix, PAGE_LINKS['spreadsheet'], 'Kakobuy Spreadsheet')} "
                f"ou la variante {_link(home_prefix, PAGE_LINKS['spreadsheet_fr'], 'spreadsheet kakobuy')} selon l'intention exacte.",
            ],
        )
        + _section(
            "Discord, coupons et confiance",
            [
                f"kakobuy discord reflète l'habitude d'échanger des liens en communauté. Ce site n'est pas un canal Discord — "
                f"il structure l'intention vers le tableur W2CLinks. Voir {_link(home_prefix, PAGE_LINKS['discord'], 'Discord & community guide')}.",
                f"Pour kakobuy coupon, vérifiez toujours les conditions dans votre compte Kakobuy. "
                f"Le guide {_link(home_prefix, PAGE_LINKS['coupon_fr'], 'Kakobuy coupon')} explique comment combiner "
                f"finds et codes sans promettre une remise fixe.",
                f"Les recherches avis kakobuy demandent une lecture prudente : comparez QC, délais et support. "
                f"Notre page {_link(home_prefix, PAGE_LINKS['avis'], 'avis kakobuy')} reste un guide indépendant, "
        ...
        + _section(
            ...
            [
                "kakobuy link correspond à des questions sur conversion de liens Taobao/1688/Weidian — même logique que "
                f"partout ailleurs : copier l'URL source, coller dans Kakobuy, attendre les photos QC. "
                f"Lire {_link(home_prefix, PAGE_LINKS['qc'], 'guide QC')} avant d'expédier.",
                f"Pour la livraison vers la France, consultez {_link(home_prefix, PAGE_LINKS['shipping'], 'livraison Kakobuy')} "
...
def article_es(home_prefix: str) -> str:
    return (
        _section(
            "Qué buscan en España con kakobuy y kakobuy spreadsheet",
            [
                "kakobuy suele ser la búsqueda de entrada: entender el agente, recargar saldo y preparar el primer pedido. "
                "kakobuy spreadsheet es más específica — el usuario quiere un listado navegable de finds, no un Excel estático.",
                f"El hub correcto es {_ext('W2CLinks spreadsheet', SPREADSHEET)}. Desde aquí filtras categoría/marca "
                f"y abres {_link(home_prefix, PAGE_LINKS['finds'], 'kakobuy finds')} para comparar entradas recientes.",
            ],
        )
        + _section(
            "Cómo comprar, Discord y cupones de envío",
            [
                f"como comprar en kakobuy es una intención tutorial clara. Sigue {_link(home_prefix, PAGE_LINKS['como_comprar'], 'cómo comprar en Kakobuy')} "
                "paso a paso: enlace → pago → QC → paquete internacional.",
                f"kakobuy discord indica búsqueda comunitaria; el finds estructurado sigue en W2CLinks — "
                f"ver {_link(home_prefix, PAGE_LINKS['discord'], 'guía Discord')}.",
                f"kakobuy shipping coupons mezcla promos de envío con dudas de peso. Revisa "
                f"{_link(home_prefix, PAGE_LINKS['shipping_coupons_es'], 'shipping coupons')} y "
                f"{_link(home_prefix, PAGE_LINKS['coupons'], 'cupones Kakobuy')} antes de pagar.",
            ],
        )
        + _section(
            "Envío, aduanas y QC para compradores españoles",
            [
                "kakobuy shipping y preguntas de plazo deben leerse junto con peso real y línea elegida. "
                f"La guía {_link(home_prefix, PAGE_LINKS['shipping'], 'envío Kakobuy')} incluye rangos EUR orientativos.",
...
def article_at(home_prefix: str) -> str:
    return (
        _section(
            "Was österreichische Kakobuy-Suchen bedeuten",
            [
                "kakobuy spreadsheet ist der häufigste produktbezogene Begriff — gemeint ist der W2CLinks-Hub mit "
                ...
                f"Lies {_link(home_prefix, PAGE_LINKS['erfahrungen'], 'Kakobuy Erfahrungen')} und "
        ...
        + _section(
            ...
            [
                f"kakobuy codes und Coupon-Intention gehören zur Checkout-Phase — siehe "
                f"{_link(home_prefix, PAGE_LINKS['codes'], 'Kakobuy codes')}. Finds zuerst auf W2CLinks wählen.",
                f"qc kakobuy betont Fotos vor dem Versand. Der "
                ...
                "dann Live-Preis im Kakobuy-Konto bestätigen.",
            ],
        )
    )
...
def article_generic_en(region: str, home_prefix: str, keywords: list[str]) -> str:
    kw_note = ", ".join(keywords[:6]) if keywords else "kakobuy spreadsheet"
    label = region_label(region)
    return (
        _section(
            f"Search intent overview for {label}",
            [
                f"Shoppers in {label} often combine brand queries (kakobuy) with product discovery "
                f"(kakobuy spreadsheet) and trust questions (is kakobuy legit / is kakobuy safe). "
                f"This guide translates those searches into actions on W2CLinks and Kakobuy.",
        ...
        + _section(
            ...
            [
                ...
                f"and paste into Kakobuy. Review QC, then read {_link(home_prefix, PAGE_LINKS['shipping'], 'shipping guide')} "
...
def country_guide_html(region: str, lang: str, home_prefix: str = "") -> str:
    ...
    from scripts.country_seo_kakobuy import country_seo_block
