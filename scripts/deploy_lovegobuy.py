#!/usr/bin/env python3
"""Deploy Lovegobuy cluster to Baota/nginx server.

Domains:
  lovegobuyspreadsheet.{es,it,nl,ca,eu}
  lovegobuy.{it,nl}
  lovegobuyguide.com

Prerequisites:
  python3 build_lovegobuy_site.py
  export LOVEGOBUY_DEPLOY_PASS='...'   # or ORIENTDIG_DEPLOY_PASS / BBDBUY_DEPLOY_PASS

Optional env:
  LOVEGOBUY_DEPLOY_HOST  (default: same as OrientDig/BBDBuy, 31.97.41.31)
  LOVEGOBUY_DEPLOY_USER  (default: root)
  LOVEGOBUY_SKIP_CERTS=1
  LOVEGOBUY_SKIP_BAOTA=1
  LOVEGOBUY_SKIP_NGINX=1
"""
...
from scripts.domains_lovegobuy import (  # noqa: E402
    LOVEGOBUY_DOMAINS,
    PLURAL_REDIRECTS,
)
...
def connect() -> paramiko.SSHClient:
    password = (
        os.environ.get("LOVEGOBUY_DEPLOY_PASS")
    ...
    if not password:
        print(
            "Set LOVEGOBUY_DEPLOY_PASS (or BBDBUY_DEPLOY_PASS / ORIENTDIG_DEPLOY_PASS)",
            file=sys.stderr,
        )
        sys.exit(1)
...
def verify(client: paramiko.SSHClient) -> None:
    for domain in CANONICAL_DOMAINS:
        needle = VERIFY_NEEDLES.get(domain, "Lovegobuy")
        cmd = (
            f"curl -sk --resolve {domain}:443:127.0.0.1 https://{domain}/ "
            f"| grep -oiE '<title>[^<]+|{needle}|lovegobuy-logo|w2clinks.com/spreadsheet|invite_code=W5RJX3' | head -8"
        )
...
def main() -> None:
    ...
    if missing:
        print(f"Run build_lovegobuy_site.py first — missing: {missing}", file=sys.stderr)
        sys.exit(1)
    ...
    if not _flag("LOVEGOBUY_SKIP_BAOTA"):
        print(f"{step}/{total} register Baota sites...")
        register_baota_sites(client)
        step += 1
    else:
        print("skip Baota registration (LOVEGOBUY_SKIP_BAOTA=1)")

    print(f"{step}/{total} upload files...")
    deploy_files(client)
    step += 1

    if not _flag("LOVEGOBUY_SKIP_CERTS"):
        print(f"{step}/{total} issue SSL certs...")
        issue_certs(client)
        step += 1
    else:
        print("skip certs (LOVEGOBUY_SKIP_CERTS=1)")

    if not _flag("LOVEGOBUY_SKIP_NGINX"):
        print(f"{step}/{total} patch nginx...")
        patch_nginx(client)
        step += 1
    else:
        print("skip nginx (LOVEGOBUY_SKIP_NGINX=1)")

    print(f"{step}/{total} verify...")
    verify(client)
    client.close()
    print("lovegobuy deploy done")
