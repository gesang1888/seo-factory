"""HTML renderer for BBDBuy Spreadsheet static pages."""
...
from scripts.deep_content_bbdbuy import merge_deep_sections
...
from scripts.hub_sections_bbdbuy import render_home_hub
from scripts.i18n_ui_bbdbuy import t as ui_t
from scripts.keyword_articles_bbdbuy import country_guide_html
from scripts.shipping_data_bbdbuy import shipping_section_html
...
from scripts.promo_banners_bbdbuy import render_promo_banners
from scripts.reddit_insights_bbdbuy import pain_points_section
...
def logo_asset(site_config: dict) -> str:
    return site_config.get("agentPlatform", {}).get(
        "logoAsset", AGENT_PLATFORM.get("logoAsset", "assets/images/bbdbuy-logo.png")
    )
REGISTER_URL = AGENT_PLATFORM["registerUrl"]
SPREADSHEET = main_spreadsheet_url()
FOOTER_DISCLAIMER = (
    "BBDBuy is the shopping agent brand referenced by this guide. "
...
LOCALE_TO_DOMAIN = {
    "en-GB": "bbdbuyspreadsheet.uk",
    "en-US": "bbdbuyspreadsheet.us",
    "nl-NL": "bbdbuyspreadsheet.nl",
    "it-IT": "bbdbuyspreadsheet.it",
    "de-DE": "bbdbuyspreadsheet.de",
    "fr-FR": "bbdbuyspreadsheet.fr",
    "x-default": "bbdbuyspreadsheet.us",
}
...
def render_hreflang(slug: str, slug_exists: dict[str, set[str]], domain: str = "") -> str:
    ...
    locale_map = HREFLANG_BBDBUY if domain in BBDBUY_DOMAINS else LOCALE_TO_DOMAIN
...
def json_ld_organization(site_config: dict, domain: str, logo_url: str) -> str:
    ...
    data = {
        "@context": "https://schema.org",
        "@type": "Organization",
        "name": f"{site_config.get('brand', 'BBDBuy')} Spreadsheet Guide",
        "url": f"https://{domain}/",
        "logo": logo_url,
        "contactPoint": {
            "@type": "ContactPoint",
...
def nav_links(slugs: list[str], current: str, home_prefix: str, lang: str) -> str:
    labels = {
        ...
        "bbdbuy-spreadsheets": "Spreadsheets",
        "bbdbuy-finds": "Finds",
        "bbdbuy-coupons": "Coupons",
        "is-bbdbuy-legit": "Legit?",
        "bbdbuy-shipping": "Shipping",
        "how-to-use-bbdbuy": "How To",
    }
    if lang == "es":
        labels.update(
            {
                "bbdbuy-spreadsheet": "Spreadsheet",
                "bbdbuy-coupons": "Cupones",
                "bbdbuy-shipping": "Envíos",
                "how-to-use-bbdbuy": "Guía",
            }
        )
    elif lang == "nl":
        labels.update(
            {
                "bbdbuy-spreadsheets": "Spreadsheets",
                "bbdbuy-coupons": "Coupons",
                "bbdbuy-shipping": "Verzending",
                "how-to-use-bbdbuy": "Handleiding",
            }
        )
    elif lang == "de":
        labels.update(
            {
                "bbdbuy-coupons": "Gutscheine",
    ...
    sheet_slug = (
        "bbdbuy-spreadsheet"
        if "bbdbuy-spreadsheet" in slug_set
        else "bbdbuy-spreadsheets"
    )
    nav_order = [
        "",
        sheet_slug,
...
def render_help_article(a: dict) -> str:
  ...
  <p class="od-policy-src"><a href="{esc(a.get("source") or PLATFORM)}" target="_blank" rel="noopener">Read on BBDBuy Help Center</a></p>
</article>"""
    ...
    <div class="header-top">
      <a href="{home_prefix or '/'}" class="brand">
        <span class="brand-mark">
          <img class="brand-logo" src="{esc(logo_href)}" width="160" height="36" alt="BBDBuy">
...
<footer class="site-footer">
  <div class="container">
    <div class="footer-grid">
      <div>
        <strong>BBDBuy Spreadsheet Guide</strong>
        <p>{contact_note}</p>
        <p>{wa_display}</p>
      ...
      <div>
        ...
        <p><a href="{home_prefix}bbdbuy-finds/">Finds</a></p>
        <p><a href="{home_prefix}bbdbuy-coupons/">Coupons</a></p>
        <p><a href="{home_prefix}is-bbdbuy-legit/">Legit?</a></p>
      </div>
      <div>
        ...
        <p><a href="{esc(REGISTER_URL)}" {ext}>BBDBuy Register</a></p>
      </div>
    </div>
...
def write_404_page(
    *,
    ...
    return render_page(
        ...
        region_label="",
        slug="__404__",
        page_data={"cta": "Open BBDBuy Spreadsheet", "cta_href": SPREADSHEET},
        locale_content={
            "title": "404 — Page Not Found | BBDBuy Spreadsheet Guide",
