...
from scripts.country_seo_bbdbuy import top_keywords
...
PAGE_LINKS = {
    "spreadsheet": "bbdbuy-spreadsheet/",
    "finds": "bbdbuy-finds/",
    "shipping": "bbdbuy-shipping/",
    "coupons": "bbdbuy-coupons/",
    "legit": "is-bbdbuy-legit/",
    "safe": "is-bbdbuy-safe/",
    "howto": "how-to-use-bbdbuy/",
    "discord": "bbdbuy-discord/",
    "coupon_fr": "bbdbuy-coupon/",
    "spreadsheet_fr": "spreadsheet-bbdbuy/",
    "avis": "avis-bbdbuy/",
    "como_comprar": "como-comprar-en-bbdbuy/",
    "shipping_coupons_es": "bbdbuy-shipping-coupons/",
    "best_spreadsheet": "best-bbdbuy-spreadsheet/",
    "calculator": "bbdbuy-shipping-calculator/",
    "payment": "bbdbuy-payment-methods/",
    "erfahrungen": "bbdbuy-erfahrungen/",
    "trustpilot": "bbdbuy-trustpilot/",
    "codes": "bbdbuy-codes/",
    "qc": "bbdbuy-qc/",
}
...
def article_us(home_prefix: str) -> str:
    return (
        _section(
            "What US shoppers mean when they search BBDBuy",
            [
                "The brand query bbdbuy usually signals intent to understand the agent itself — registration, "
                "balance top-ups, warehouse flow and whether the platform fits a first purchase. "
                "It is different from bbdbuy spreadsheet, which almost always means a browsable finds list.",
                "US search volume also clusters around trust queries such as is bbdbuy legit and is bbdbuy safe. "
        ...
        + _section(
            ...
            [
                f"bbdbuy spreadsheet and best bbdbuy spreadsheet map to the same practical action: open the live "
        ...
        + _section(
            ...
            [
                "bbdbuy shipping calculator searches reflect parcel planning anxiety: weight, line choice, and whether "
                f"delivery fits a deadline. Read {_link(home_prefix, PAGE_LINKS['shipping'], 'US shipping notes')} for "
                f"estimated line ranges, then confirm live quotes inside BBDBuy before paying.",
                "Coupon-related queries such as bbdbuy coupon and bbdbuy coupon codes change frequently. "
        ...
        + _section(
            ...
            [
                "1) Shortlist on W2CLinks. 2) Paste links into BBDBuy. 3) Review QC photos. "
                "4) Compare shipping lines with realistic customs buffer. 5) Track import milestones. "
                f"Full steps live in {_link(home_prefix, PAGE_LINKS['howto'], 'how to use BBDBuy')}.",
            ],
        )
    )
...
def article_fr(home_prefix: str) -> str:
    return (
        _section(
            "Ce que les acheteurs français cherchent avec « bbdbuy »",
            [
                "En France, la requête bbdbuy mélange curiosité sur la plateforme agent et envie de trouver des finds "
                ...
                "Les recherches bbdbuy spreadsheet et spreadsheet bbdbuy (ordre inversé) pointent vers le même "
                f"besoin : filtrer des listes communautaires. Utilisez {_link(home_prefix, PAGE_LINKS['spreadsheet'], 'BBDBuy Spreadsheet')} "
                f"ou la variante {_link(home_prefix, PAGE_LINKS['spreadsheet_fr'], 'spreadsheet bbdbuy')} selon l'intention exacte.",
            ],
        )
        + _section(
            "Discord, coupons et confiance",
            [
                f"bbdbuy discord reflète l'habitude d'échanger des liens en communauté. Ce site n'est pas un canal Discord — "
                f"il structure l'intention vers le tableur W2CLinks. Voir {_link(home_prefix, PAGE_LINKS['discord'], 'Discord & community guide')}.",
                f"Pour bbdbuy coupon, vérifiez toujours les conditions dans votre compte BBDBuy. "
                f"Le guide {_link(home_prefix, PAGE_LINKS['coupon_fr'], 'BBDBuy coupon')} explique comment combiner "
                f"finds et codes sans promettre une remise fixe.",
                f"Les recherches avis bbdbuy demandent une lecture prudente : comparez QC, délais et support. "
                f"Notre page {_link(home_prefix, PAGE_LINKS['avis'], 'avis bbdbuy')} reste un guide indépendant, "
        ...
        + _section(
            ...
            [
                "bbdbuy link correspond à des questions sur conversion de liens Taobao/1688/Weidian — même logique que "
                f"partout ailleurs : copier l'URL source, coller dans BBDBuy, attendre les photos QC. "
                f"Lire {_link(home_prefix, PAGE_LINKS['qc'], 'guide QC')} avant d'expédier.",
                f"Pour la livraison vers la France, consultez {_link(home_prefix, PAGE_LINKS['shipping'], 'livraison BBDBuy')} "
...
def article_es(home_prefix: str) -> str:
    return (
        _section(
            "Qué buscan en España con bbdbuy y bbdbuy spreadsheet",
            [
                "bbdbuy suele ser la búsqueda de entrada: entender el agente, recargar saldo y preparar el primer pedido. "
                "bbdbuy spreadsheet es más específica — el usuario quiere un listado navegable de finds, no un Excel estático.",
                f"El hub correcto es {_ext('W2CLinks spreadsheet', SPREADSHEET)}. Desde aquí filtras categoría/marca "
                f"y abres {_link(home_prefix, PAGE_LINKS['finds'], 'bbdbuy finds')} para comparar entradas recientes.",
            ],
        )
        + _section(
            "Cómo comprar, Discord y cupones de envío",
            [
                f"como comprar en bbdbuy es una intención tutorial clara. Sigue {_link(home_prefix, PAGE_LINKS['como_comprar'], 'cómo comprar en BBDBuy')} "
                "paso a paso: enlace → pago → QC → paquete internacional.",
                f"bbdbuy discord indica búsqueda comunitaria; el finds estructurado sigue en W2CLinks — "
                f"ver {_link(home_prefix, PAGE_LINKS['discord'], 'guía Discord')}.",
                f"bbdbuy shipping coupons mezcla promos de envío con dudas de peso. Revisa "
                f"{_link(home_prefix, PAGE_LINKS['shipping_coupons_es'], 'shipping coupons')} y "
                f"{_link(home_prefix, PAGE_LINKS['coupons'], 'cupones BBDBuy')} antes de pagar.",
            ],
        )
        + _section(
            "Envío, aduanas y QC para compradores españoles",
            [
                "bbdbuy shipping y preguntas de plazo deben leerse junto con peso real y línea elegida. "
                f"La guía {_link(home_prefix, PAGE_LINKS['shipping'], 'envío BBDBuy')} incluye rangos EUR orientativos.",
...
def article_at(home_prefix: str) -> str:
    return (
        _section(
            "Was österreichische BBDBuy-Suchen bedeuten",
            [
                "bbdbuy spreadsheet ist der häufigste produktbezogene Begriff — gemeint ist der W2CLinks-Hub mit "
                ...
                f"Lies {_link(home_prefix, PAGE_LINKS['erfahrungen'], 'BBDBuy Erfahrungen')} und "
        ...
        + _section(
            ...
            [
                f"bbdbuy codes und Coupon-Intention gehören zur Checkout-Phase — siehe "
                f"{_link(home_prefix, PAGE_LINKS['codes'], 'BBDBuy codes')}. Finds zuerst auf W2CLinks wählen.",
                f"qc bbdbuy betont Fotos vor dem Versand. Der "
                ...
                "dann Live-Preis im BBDBuy-Konto bestätigen.",
            ],
        )
    )
...
def article_generic_en(region: str, home_prefix: str, keywords: list[str]) -> str:
    kw_note = ", ".join(keywords[:6]) if keywords else "bbdbuy spreadsheet"
    label = region_label(region)
    return (
        _section(
            f"Search intent overview for {label}",
            [
                f"Shoppers in {label} often combine brand queries (bbdbuy) with product discovery "
                f"(bbdbuy spreadsheet) and trust questions (is bbdbuy legit / is bbdbuy safe). "
                f"This guide translates those searches into actions on W2CLinks and BBDBuy.",
        ...
        + _section(
            ...
            [
                ...
                f"and paste into BBDBuy. Review QC, then read {_link(home_prefix, PAGE_LINKS['shipping'], 'shipping guide')} "
...
def country_guide_html(region: str, lang: str, home_prefix: str = "") -> str:
    ...
    from scripts.country_seo_bbdbuy import country_seo_block
