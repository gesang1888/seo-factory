...
def _base_spreadsheet(region: str, lang: str) -> list[tuple[str, str]]:
    en = [
        (
            "What orientdig spreadsheet searchers actually want",
            "Most people typing orientdig spreadsheet are not looking for a downloadable Excel file on a random blog. "
        ...
        (
            ...
            "When a find looks promising, open the listing, copy the source link, and paste it into OrientDig. "
        ...
        (
            ...
            "size charts, or OrientDig warehouse inspection. Treat finds as a starting point, not a guarantee.",
        ),
        (
        ...
        (
            ...
            f"After you pick items on W2CLinks, verify OrientDig promotions in your account, read QC photos carefully, "
    ...
    if lang == "es":
        return [
            (
                "Qué buscan realmente con orientdig spreadsheet",
            ...
            (
                ...
                "y pégalo en OrientDig. Paga, espera inbound, revisa QC y solo entonces crea el paquete internacional.",
            ),
            (
                "Errores comunes",
    ...
    if lang == "fr":
        return [
            (
                "Ce que les recherches orientdig spreadsheet signifient",
            ...
            (
                "Workflow en étapes",
                "Ouvrez le tableur, triez par newest, filtrez catégorie/marque, copiez le lien source dans OrientDig, "
    ...
    if lang == "de":
        return [
            (
                "Was Nutzer mit orientdig spreadsheet meinen",
            ...
            (
                "Schritt-für-Schritt",
                "Spreadsheet öffnen, nach newest sortieren, Kategorie/Marke filtern, Link in OrientDig einfügen, "
...
def _shipping_deep(region: str, lang: str) -> list[tuple[str, str]]:
    en = [
        (
            ...
            f"International shipping does not start when you pay for a Taobao/1688 item. Items land at the OrientDig "
        ...
        (
            ...
            "Customs delays are often unrelated to OrientDig itself — they happen at import screening. "
...
def _trust_deep(lang: str) -> list[tuple[str, str]]:
    return [
        (
            ...
            f"OrientDig ({PLATFORM}) purchases from Chinese marketplaces on your behalf. "
        ...
        (
            ...
            "and open live finds — then complete orders on OrientDig.",
        ),
    ]
...
def _coupons_deep(lang: str) -> list[tuple[str, str]]:
    return [
        (
            "How OrientDig coupons actually work",
            ...
            "Always verify eligibility inside your OrientDig account before paying.",
        ),
        (
        ...
        (
            "1300CNY bonus context",
            "Community mentions of 1300CNY registration or shipping bonuses refer to OrientDig policy pages — "
            "read live help-center articles for current thresholds, expiration, and whether the bonus applies "
            "to shipping, balance, or first parcel only.",
        ),
    ]
...
def _howto_deep(lang: str) -> list[tuple[str, str]]:
    return [
        (
            ...
            f"1) Discover on W2CLinks spreadsheet. 2) Copy product URL. 3) Paste into OrientDig buy form. "
...
SLUG_HANDLERS = {
    ...
    "orientdig-spreadsheet": lambda r, l: _base_spreadsheet(r, l),
    "orientdig-spreadsheets": lambda r, l: _base_spreadsheet(r, l),
    "orientdig-finds": lambda r, l: _base_spreadsheet(r, l) + [
        (
    ...
    "orientdig-shipping": lambda r, l: _shipping_deep(r, l),
    "orientdig-shipping-calculator": lambda r, l: _shipping_deep(r, l),
    "orientdig-shipping-coupons": lambda r, l: _shipping_deep(r, l) + _coupons_deep(l),
    "livraison-orientdig": lambda r, l: _shipping_deep(r, l),
    "orientdig-coupons": lambda r, l: _coupons_deep(l),
    "orientdig-coupon": lambda r, l: _coupons_deep(l),
    "orientdig-codes": lambda r, l: _coupons_deep(l),
    "orientdig-qc": lambda r, l: _qc_deep(l),
    "qc-orientdig": lambda r, l: _qc_deep(l),
    "is-orientdig-legit": lambda r, l: _trust_deep(l),
    "is-orientdig-safe": lambda r, l: _trust_deep(l),
    "how-to-use-orientdig": lambda r, l: _howto_deep(l),
    "como-comprar-en-orientdig": lambda r, l: _howto_deep(l),
    "orientdig-review": lambda r, l: _trust_deep(l),
    "avis-orientdig": lambda r, l: _trust_deep(l),
    "best-orientdig-spreadsheet": lambda r, l: _base_spreadsheet(r, l),
}
