...
def _base_spreadsheet(region: str, lang: str) -> list[tuple[str, str]]:
    en = [
        (
            "What bbdbuy spreadsheet searchers actually want",
            "Most people typing bbdbuy spreadsheet are not looking for a downloadable Excel file on a random blog. "
        ...
        (
            ...
            "When a find looks promising, open the listing, copy the source link, and paste it into BBDBuy. "
        ...
        (
            ...
            "size charts, or BBDBuy warehouse inspection. Treat finds as a starting point, not a guarantee.",
        ),
        (
        ...
        (
            ...
            f"After you pick items on W2CLinks, verify BBDBuy promotions in your account, read QC photos carefully, "
    ...
    if lang == "es":
        return [
            (
                "Qué buscan realmente con bbdbuy spreadsheet",
            ...
            (
                ...
                "y pégalo en BBDBuy. Paga, espera inbound, revisa QC y solo entonces crea el paquete internacional.",
            ),
            (
                "Errores comunes",
    ...
    if lang == "fr":
        return [
            (
                "Ce que les recherches bbdbuy spreadsheet signifient",
            ...
            (
                "Workflow en étapes",
                "Ouvrez le tableur, triez par newest, filtrez catégorie/marque, copiez le lien source dans BBDBuy, "
    ...
    if lang == "de":
        return [
            (
                "Was Nutzer mit bbdbuy spreadsheet meinen",
            ...
            (
                "Schritt-für-Schritt",
                "Spreadsheet öffnen, nach newest sortieren, Kategorie/Marke filtern, Link in BBDBuy einfügen, "
...
def _shipping_deep(region: str, lang: str) -> list[tuple[str, str]]:
    en = [
        (
            ...
            f"International shipping does not start when you pay for a Taobao/1688 item. Items land at the BBDBuy "
        ...
        (
            ...
            "Customs delays are often unrelated to BBDBuy itself — they happen at import screening. "
...
def _trust_deep(lang: str) -> list[tuple[str, str]]:
    return [
        (
            ...
            f"BBDBuy ({PLATFORM}) purchases from Chinese marketplaces on your behalf. "
        ...
        (
            ...
            "and open live finds — then complete orders on BBDBuy.",
        ),
    ]
...
def _coupons_deep(lang: str) -> list[tuple[str, str]]:
    return [
        (
            "How BBDBuy coupons actually work",
            ...
            "Always verify eligibility inside your BBDBuy account before paying.",
        ),
        (
        ...
        (
            ...
            "Community mentions of 1300CNY registration or shipping bonuses refer to BBDBuy policy pages — "
...
def _howto_deep(lang: str) -> list[tuple[str, str]]:
    return [
        (
            ...
            f"1) Discover on W2CLinks spreadsheet. 2) Copy product URL. 3) Paste into BBDBuy buy form. "
...
SLUG_HANDLERS = {
    ...
    "bbdbuy-spreadsheet": lambda r, l: _base_spreadsheet(r, l),
    "bbdbuy-spreadsheets": lambda r, l: _base_spreadsheet(r, l),
    "bbdbuy-finds": lambda r, l: _base_spreadsheet(r, l) + [
        (
    ...
    "bbdbuy-shipping": lambda r, l: _shipping_deep(r, l),
    "bbdbuy-shipping-calculator": lambda r, l: _shipping_deep(r, l),
    "bbdbuy-shipping-coupons": lambda r, l: _shipping_deep(r, l) + _coupons_deep(l),
    "livraison-bbdbuy": lambda r, l: _shipping_deep(r, l),
    "bbdbuy-coupons": lambda r, l: _coupons_deep(l),
    "bbdbuy-coupon": lambda r, l: _coupons_deep(l),
    "bbdbuy-codes": lambda r, l: _coupons_deep(l),
    "bbdbuy-qc": lambda r, l: _qc_deep(l),
    "qc-bbdbuy": lambda r, l: _qc_deep(l),
    "is-bbdbuy-legit": lambda r, l: _trust_deep(l),
    "is-bbdbuy-safe": lambda r, l: _trust_deep(l),
    "how-to-use-bbdbuy": lambda r, l: _howto_deep(l),
    "como-comprar-en-bbdbuy": lambda r, l: _howto_deep(l),
    "bbdbuy-review": lambda r, l: _trust_deep(l),
    "avis-bbdbuy": lambda r, l: _trust_deep(l),
    "best-bbdbuy-spreadsheet": lambda r, l: _base_spreadsheet(r, l),
}
