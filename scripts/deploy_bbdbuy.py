#!/usr/bin/env python3
"""Deploy BBDBuy cluster (bbdbuy.uk/us/ca/it) to Baota/nginx server."""
...
HOST = os.environ.get("BBDBUY_DEPLOY_HOST", os.environ.get("ORIENTDIG_DEPLOY_HOST", "31.97.41.31"))
USER = os.environ.get("BBDBUY_DEPLOY_USER", os.environ.get("ORIENTDIG_DEPLOY_USER", "root"))


def connect() -> paramiko.SSHClient:
    password = os.environ.get("BBDBUY_DEPLOY_PASS") or os.environ.get("ORIENTDIG_DEPLOY_PASS")
    if not password:
        print("Set BBDBUY_DEPLOY_PASS", file=sys.stderr)
        sys.exit(1)
...
def register_baota_sites(client: paramiko.SSHClient) -> None:
    domains_json = ",".join(f'"{d}"' for d in BBDBUY_DOMAINS)
    script = f"""
cd /www/server/panel
...
def verify(client: paramiko.SSHClient) -> None:
    for domain in BBDBUY_DOMAINS:
        cmd = (
            ...
            f"| grep -oiE '<title>[^<]+|BBDBuy|bbdbuy-logo|w2clinks.com/spreadsheet|447856544534' | head -8"
        )
...
def main() -> None:
    missing = [d for d in BBDBUY_DOMAINS if not (DIST / d).is_dir()]
    if missing:
        print(f"Run build_bbdbuy_site.py first — missing: {missing}", file=sys.stderr)
        sys.exit(1)
    client = connect()
