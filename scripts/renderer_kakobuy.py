"""HTML renderer for Kakobuy Spreadsheet static pages."""
...
from scripts.deep_content_kakobuy import merge_deep_sections
...
from scripts.hub_sections_kakobuy import render_home_hub
from scripts.i18n_ui_kakobuy import t as ui_t
from scripts.keyword_articles_kakobuy import country_guide_html
from scripts.shipping_data_kakobuy import shipping_section_html
...
from scripts.promo_banners_kakobuy import render_promo_banners
from scripts.reddit_insights_kakobuy import pain_points_section
...
def asset_query() -> str:
    ...
    stamps = []
    for name in ("orientdig-theme.css", "kakobuy-theme.css", "site.js"):
...
def logo_asset(site_config: dict) -> str:
    return site_config.get("agentPlatform", {}).get(
        "logoAsset", AGENT_PLATFORM.get("logoAsset", "assets/images/kakobuy-logo.png")
    )
REGISTER_URL = AGENT_PLATFORM["registerUrl"]
SPREADSHEET = main_spreadsheet_url()
FOOTER_DISCLAIMER = (
    "Kakobuy is the shopping agent brand referenced by this guide. "
...
COUPON_SLUGS = {
    ...
    "kakobuy-coupon-code",
    "kakobuy-codes",
    "kakobuy-coupon-codes",
    "kakobuy-shipping-coupons",
}
...
def render_hreflang(slug: str, slug_exists: dict[str, set[str]], domain: str = "") -> str:
    ...
    locale_map = HREFLANG_KAKOBUY if domain in KAKOBUY_DOMAINS else LOCALE_TO_DOMAIN
...
def json_ld_organization(site_config: dict, domain: str, logo_url: str) -> str:
    ...
    data = {
        "@context": "https://schema.org",
        "@type": "Organization",
        "name": f"{site_config.get('brand', 'Kakobuy')} Spreadsheet Guide",
        "url": f"https://{domain}/",
        "logo": logo_url,
        "contactPoint": {
            "@type": "ContactPoint",
...
def nav_links(slugs: list[str], current: str, home_prefix: str, lang: str) -> str:
    labels = {
        ...
        "kakobuy-spreadsheets": "Spreadsheets",
        "kakobuy-finds": "Finds",
        "kakobuy-coupons": "Coupons",
        "is-kakobuy-legit": "Legit?",
        "kakobuy-shipping": "Shipping",
        "how-to-use-kakobuy": "How To",
    }
    if lang == "es":
        labels.update(
            {
                "kakobuy-spreadsheet": "Spreadsheet",
                "kakobuy-coupons": "Cupones",
                "kakobuy-shipping": "Envíos",
        ...
        labels.update(
            {
                "kakobuy-coupons": "Gutscheine",
                "kakobuy-shipping": "Versand",
                "how-to-use-kakobuy": "Anleitung",
            }
        )
    elif lang == "it":
        labels.update(
            {
                "kakobuy-coupons": "Coupon",
                "kakobuy-shipping": "Spedizione",
                "how-to-use-kakobuy": "Guida",
            }
        )
    elif lang == "fr":
        labels.update(
    ...
    sheet_slug = (
        "kakobuy-spreadsheet"
        if "kakobuy-spreadsheet" in slug_set
        else "kakobuy-spreadsheets"
    )
    nav_order = [
        "",
        sheet_slug,
...
def render_help_article(a: dict) -> str:
  ...
  <p class="od-policy-src"><a href="{esc(a.get("source") or PLATFORM)}" target="_blank" rel="noopener">Read on Kakobuy Help Center</a></p>
</article>"""
...
def render_page(
    *,
    ...
    brand_css_href = f"{home_prefix}assets/css/kakobuy-theme.css{asset_q}"
    ...
    <div class="header-top">
      <a href="{home_prefix or '/'}" class="brand">
        <span class="brand-mark">
          <img class="brand-logo" src="{esc(logo_href)}" width="160" height="36" alt="Kakobuy">
...
<footer class="site-footer">
  <div class="container">
    <div class="footer-grid">
      <div>
        <strong>Kakobuy Spreadsheet Guide</strong>
        <p>{contact_note}</p>
        <p>{wa_display}</p>
      ...
      <div>
        ...
        <p><a href="{home_prefix}kakobuy-finds/">Finds</a></p>
        <p><a href="{home_prefix}kakobuy-coupons/">Coupons</a></p>
        <p><a href="{home_prefix}is-kakobuy-legit/">Legit?</a></p>
      </div>
      <div>
        ...
        <p><a href="{esc(REGISTER_URL)}" {ext}>Kakobuy Register</a></p>
      </div>
    </div>
...
def write_404_page(
    *,
    ...
    return render_page(
        ...
        region_label="",
        slug="__404__",
        page_data={"cta": "Open Kakobuy Spreadsheet", "cta_href": SPREADSHEET},
        locale_content={
            "title": "404 — Page Not Found | Kakobuy Spreadsheet Guide",
...
def copy_assets(dist_domain_dir: Path, template_root: Path) -> None:
    ...
    for css_name in ("orientdig-theme.css", "kakobuy-theme.css"):
