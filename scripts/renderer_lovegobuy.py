"""HTML renderer for Lovegobuy Spreadsheet static pages."""
...
from scripts.deep_content_lovegobuy import merge_deep_sections
...
from scripts.hub_sections_lovegobuy import render_home_hub
from scripts.i18n_ui_lovegobuy import t as ui_t
from scripts.keyword_articles_lovegobuy import country_guide_html
from scripts.shipping_data_lovegobuy import shipping_section_html
...
from scripts.promo_banners_lovegobuy import render_promo_banners
from scripts.reddit_insights_lovegobuy import pain_points_section
...
def asset_query() -> str:
    ...
    stamps = []
    for name in ("orientdig-theme.css", "lovegobuy-theme.css", "site.js"):
...
def logo_asset(site_config: dict) -> str:
    return site_config.get("agentPlatform", {}).get(
        "logoAsset", AGENT_PLATFORM.get("logoAsset", "assets/images/lovegobuy-logo.png")
    )
REGISTER_URL = AGENT_PLATFORM["registerUrl"]
SPREADSHEET = main_spreadsheet_url()
FOOTER_DISCLAIMER = (
    "Lovegobuy is the shopping agent brand referenced by this guide. "
...
PRODUCT_SLUGS = {
    ...
    "lovegobuy-finds",
    "spreadsheet-lovegobuy",
    "best-lovegobuy-spreadsheet",
    "lovegobuy-spreadsheet-2026",
}

SHIPPING_SLUGS = {
    "lovegobuy-shipping",
    "lovegobuy-shipping-calculator",
    "how-long-does-lovegobuy-take-to-ship",
    "livraison-lovegobuy",
    "lovegobuy-tracking",
}
...
COUPON_SLUGS = {
    ...
    "lovegobuy-codes",
    "lovegobuy-coupon-codes",
    "lovegobuy-shipping-coupons",
}
...
def render_hreflang(slug: str, slug_exists: dict[str, set[str]], domain: str = "") -> str:
    ...
    locale_map = HREFLANG_LOVEGOBUY if domain in LOVEGOBUY_DOMAINS else LOCALE_TO_DOMAIN
...
def json_ld_organization(site_config: dict, domain: str, logo_url: str) -> str:
    ...
    data = {
        "@context": "https://schema.org",
        "@type": "Organization",
        "name": f"{site_config.get('brand', 'Lovegobuy')} Spreadsheet Guide",
        "url": f"https://{domain}/",
        "logo": logo_url,
        "contactPoint": {
            "@type": "ContactPoint",
...
def hero_block(
  ...
  <p class="hero-ctas">
    ...
    {platform_link}
        ...
        return f"""<section class="hero lg-hero">
...
def nav_links(slugs: list[str], current: str, home_prefix: str, lang: str) -> str:
    labels = {
        ...
        "lovegobuy-spreadsheets": "Spreadsheets",
        "lovegobuy-finds": "Finds",
        "lovegobuy-coupons": "Coupons",
        "is-lovegobuy-legit": "Legit?",
        "lovegobuy-shipping": "Shipping",
        "how-to-use-lovegobuy": "How To",
    }
    if lang == "es":
        labels.update(
            {
                "lovegobuy-spreadsheet": "Spreadsheet",
                "lovegobuy-coupons": "Cupones",
                "lovegobuy-shipping": "Envíos",
                "how-to-use-lovegobuy": "Guía",
            }
        )
    elif lang == "nl":
        labels.update(
            {
                "lovegobuy-spreadsheets": "Spreadsheets",
                "lovegobuy-coupons": "Coupons",
                "lovegobuy-shipping": "Verzending",
                "how-to-use-lovegobuy": "Handleiding",
            }
        )
    elif lang == "de":
        labels.update(
            {
                "lovegobuy-coupons": "Gutscheine",
                "lovegobuy-shipping": "Versand",
                "how-to-use-lovegobuy": "Anleitung",
            }
        )
    elif lang == "it":
        labels.update(
            {
                "lovegobuy-coupons": "Coupon",
                "lovegobuy-shipping": "Spedizione",
                "how-to-use-lovegobuy": "Guida",
            }
        )
    elif lang == "fr":
        labels.update(
            {
                "lovegobuy-coupons": "Coupons",
                "lovegobuy-shipping": "Livraison",
                "how-to-use-lovegobuy": "Guide",
            }
        )
    ...
    sheet_slug = (
        "lovegobuy-spreadsheet"
        if "lovegobuy-spreadsheet" in slug_set
        else "lovegobuy-spreadsheets"
    )
    nav_order = [
        "",
        sheet_slug,
...
def render_help_article(a: dict) -> str:
    ...
    for src in (a.get("images") or [])[:2]:
        if not src.startswith("http"):
            src = PLATFORM + src if src.startswith("/") else src
        imgs += (
  ...
  <p class="od-policy-src"><a href="{esc(a.get("source") or PLATFORM)}" target="_blank" rel="noopener">Read on Lovegobuy Help Center</a></p>
</article>"""
...
def render_page(
    *,
    ...
    asset_q = asset_query()
    base_css_href = f"{home_prefix}assets/css/orientdig-theme.css{asset_q}"
    brand_css_href = f"{home_prefix}assets/css/lovegobuy-theme.css{asset_q}"
    ...
    logo_path = logo_asset(site_config)
    logo_href = f"{home_prefix}{logo_path.lstrip('/')}"
    favicon_path = site_config.get("agentPlatform", {}).get("faviconAsset", "assets/images/favicon.ico")
    favicon_href = f"{home_prefix}{favicon_path.lstrip('/')}"
    logo_abs = f"https://{domain}/{logo_path.lstrip('/')}"
...
<link rel="stylesheet" href="{base_css_href}">
<link rel="stylesheet" href="{brand_css_href}">
...
<footer class="site-footer">
  <div class="container">
    <div class="footer-grid">
      <div>
        <strong>Lovegobuy Spreadsheet Guide</strong>
        <p>{contact_note}</p>
        <p>{wa_display}</p>
      ...
      <div>
        ...
        <p><a href="{home_prefix}lovegobuy-finds/">Finds</a></p>
        <p><a href="{home_prefix}lovegobuy-coupons/">Coupons</a></p>
        <p><a href="{home_prefix}is-lovegobuy-legit/">Legit?</a></p>
      </div>
      <div>
        ...
        <p><a href="{esc(REGISTER_URL)}" {ext}>Lovegobuy Register</a></p>
      </div>
    </div>
...
def write_404_page(
    *,
    ...
    return render_page(
        ...
        region_label="",
        slug="__404__",
        page_data={"cta": "Open Lovegobuy Spreadsheet", "cta_href": SPREADSHEET},
        locale_content={
            "title": "404 — Page Not Found | Lovegobuy Spreadsheet Guide",
...
def copy_assets(dist_domain_dir: Path, template_root: Path) -> None:
    ...
    for css_name in ("orientdig-theme.css", "lovegobuy-theme.css"):
        src = template_root / css_name
        if src.is_file():
            (css_dir / css_name).write_text(src.read_text(encoding="utf-8"), encoding="utf-8")
    fav = template_root / "assets" / "images" / "favicon.ico"
    if fav.is_file():
        img_dir = dist_domain_dir / "assets" / "images"
        img_dir.mkdir(parents=True, exist_ok=True)
        (img_dir / "favicon.ico").write_bytes(fav.read_bytes())
