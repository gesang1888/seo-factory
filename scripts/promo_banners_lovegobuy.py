"""Lovegobuy home promos: synced official activities + local guide cards (shipping, guide)."""

from __future__ import annotations

from html import escape as esc

from scripts.fetch_lovegobuy_activities import load_activities
from scripts.i18n_ui_lovegobuy import t as ui_t
from scripts.link_helpers_lovegobuy import AGENT_PLATFORM
...
LOCAL_GUIDE_ITEMS = [
    {
        ...
        "slugs": {
            "en": "/lovegobuy-shipping/",
            "nl": "/lovegobuy-verzending/",
            "es": "/envio-lovegobuy-espana/",
            "it": "/spedizione-lovegobuy/",
        },
        "region_slugs": {"CA": "/lovegobuy-shipping-to-canada/"},
    ...
    },
    {
        ...
        "slugs": {lang: "/how-to-use-lovegobuy/" for lang in ("en", "nl", "es", "it")},
...
def _lang_ui(lang: str) -> str:
    return lang if lang in ("nl", "es", "it") else "en"
...
def render_top_promo_strip(lang: str) -> str:
    """Announcement strip synced with lovegobuy.com new-user coupon promo."""
    ui = _lang_ui(lang)
    text = ui_t(ui, "promo_strip_text")
    cta = ui_t(ui, "promo_strip_cta")
    return f"""<div class="lg-promo"><div class="container"><span>{esc(text)}</span> · <a href="{esc(REGISTER)}" target="_blank" rel="sponsored noopener">{esc(cta)}</a></div></div>"""


def render_official_activities(home_prefix: str, lang: str, region: str) -> str:
    """Official banners synced from lovegobuy.com homepage carousel."""
    ui = _lang_ui(lang)
    cards = []
    for act in load_activities():
        ...
        if act.get("kind") == "register_coupon":
            href = REGISTER
  ...
  <img src="{esc(img)}" alt="{esc(title)}" width="460" height="245" loading="lazy" decoding="async" referrerpolicy="no-referrer">
</a>"""
        )
        if len(cards) >= 3:
            break
